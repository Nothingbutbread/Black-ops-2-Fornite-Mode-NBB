...
addItemToInventory(index, item) {
	...
	// Error Check
	if (!item.slotfilled) {
		self iprintln("^1Failed item.slotfilled check. Tried to add an empty item!");
		return olditem;
	}
	replace = false;
	...
	for(a = 0; a < 5; a++) {
		if (!self.inv[a].slotfilled) {
			index = a;
			//self iprintln("Will replace empty item at slot " + a);
			break;
		}
	}
	
	//self iprintln("LUI: " + self.lastusedinvslotindex + " | index: " + index);
	...
	if (self.lastusedinvslotindex == index && self.inv[index].weapon == self.activeweapon) {
		//self iprintln("AITI: Will Replace Item In Hand!");
		replace = true;
	}
	...
	if (self.inv[index].slotfilled) {
		self.tempitem = self.inv[index];
		if (replace) {
			self.ammotypes[self.activetype] += self getweaponammostock(self.activeweapon);
		}
	}
	
	self.inv[index].weapon = item.weapon;
	self.inv[index].clip = item.clip;
	self.inv[index].rarity = item.rarity;
	self.inv[index].ammotype = item.ammotype;
	self.inv[index].isweapon = item.isweapon;
