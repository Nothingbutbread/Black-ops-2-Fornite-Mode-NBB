...
SpawnSupplyDrop(sorigin, eorigin, dur) {
    ...
    for(x = 0; x < 5; x++) {
        if (level.flyexplosives) {
            ...
            item[x] = createAmmoItem(2, GetRandomAmmoAmmout(2));
        } else {
...
MoveBattleBus() {
    ...
    for(x = 0; x < level.battlebustimetotarget; x++) {
        if (x < 6) {
            iprintln("Battle bus last stop in " + (level.battlebustimetotarget - x) + " seconds!"); 
        }
        wait 1;
    }
    foreach(player in level.players) {
...
DetachWhenReady(time) {
    ...
    wait time;
    self setperk("specialty_fallheight");
    self fadeInItemToolTip("Press [{+usereload}] to Jump off the Battle Bus");
    while(true) {
