CreatePlayerHUDS()
{
	...
	self.fortHUDS[5] = self SpawnShader("menu_mp_weapons_ballistic_knife", 120, 320, 40, 40, (1,1,1), .8, 10);
	self.fortHUDS[6] = self SpawnShader("menu_mp_weapons_ballistic_knife", 165, 320, 40, 40, (1,1,1), .8, 10);
	self.fortHUDS[7] = self SpawnShader("menu_mp_weapons_ballistic_knife", 210, 320, 40, 40, (1,1,1), .8, 10);
	self.fortHUDS[8] = self SpawnShader("menu_mp_weapons_ballistic_knife", 255, 320, 40, 40, (1,1,1), .8, 10);
	self.fortHUDS[9] = self SpawnShader("menu_mp_weapons_ballistic_knife", 300, 320, 40, 40, (1,1,1), .8, 10);
...
updateHUDRemoveItemFromInv(index)
{
	...
	self.fortHUDS[index + 5] setshader("menu_mp_weapons_ballistic_knife", 40, 40);
}
...
OpenInventoryGUIBind()
{
	...
	while(true) {
		if (self actionslotonebuttonpressed() && !self.menuopen && self.status > 2) {
			...
			//self iprintln("Weapon Held changed to inventory slot: " + self.selectorpos);
...
OpenChestGUI(data, type)
{
	...
	while(self.menuopen) {
		if (self actionslotfourbuttonpressed() && self.curmenu == 0 && self.selectorpos < 4) { //Right
			...
			self.fortHUDS[14].x += 45;
		} else if (self actionslotthreebuttonpressed() && self.curmenu == 0 && self.selectorpos > 0) { // Left
			self.selectorpos--;
			self.fortHUDS[14] moveOverTime(.05);
			self.fortHUDS[14].x -= 45;
...
keyBinds()
{
	...
	self.menutext = "Utility Menu:\nBuild Wall\nBuild Ramp\nGet Unstuck\nAdjust Menu Controls\nDelete All Ramps/Walls\nDelete Last Placed Item";
	self.fortHUDS[12] setSafeText(self.menutext);
	// 0 = Classic, 1 = AutoChange To, 2 = [Default] AutoClose
	if (self.useinventorymod == 1) {
		self updateControlsInfo("[{+actionslot 3}] / [{+actionslot 4}] Move Selector and Select\n[{+melee}] / [{+usereload}]Close Menu");
	} else if (self.useinventorymod == 2) {
		self updateControlsInfo("[{+actionslot 3}] / [{+actionslot 4}] Move Selector\n[{+usereload}] Select item and Close Menu\n[{+melee}] Close Menu");
	} else {
		self updateControlsInfo("[{+actionslot 3}] / [{+actionslot 4}] Move Selector\n[{+usereload}] Select item\n[{+melee}] Close Menu");
	}
	while(self.menuopen) {
		if (self actionslotfourbuttonpressed() && self.curmenu == 0 && self.selectorpos < 4) { //Right
			self.selectorpos++;
